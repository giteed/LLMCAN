...
def preprocess_query(user_input):
    ...
    system_prompt = f"""Проанализируй запрос пользователя, исправь возможные ошибки и сформулируй до трех связанных поисковых запросов для расширения контекста учитывая текущую дату и время {current_datetime}.".
...
2. [запрос 2]
3. [запрос 3]
Составь Инструкцию для обработки результатов:
4. [Тут детальная инструкция по обработке и форматированию результатов поиска для LLM модели]"""
