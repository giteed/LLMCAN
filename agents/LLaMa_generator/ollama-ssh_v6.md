...
```mermaid
flowchart TD
    A["Входящие заказы (01_user_message.txt): Здравствуйте, я хотел бы заказать пиццу Маргарита с дополнительным сыром, апельсиновый сок и свежий салат."]
    B["Системный промпт (02_sys_prompt.txt): Форматируйте ответ, указывайте номер заказа, перечень позиций и контактные данные."]
    C["Шаблон ответа (03_response_template.txt): Номер заказа, список позиций, сроки выполнения."]
    D["Генерация ответа (Ollama)"]
    E["Запись ответа с уникальным номером (04_model_response.txt)"]
    F["Маршрутизация: отправка ответа пользователю или в отдел заказов"]
