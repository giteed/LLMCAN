...
```mermaid
flowchart TD
    A[Входящие заказы<br/>(01_user_message.txt)<br/><i>"Здравствуйте, я хотел бы заказать пиццу 'Маргарита' с дополнительным сыром, апельсиновый сок и свежий салат."</i>]
    B[Системный промпт<br/>(02_sys_prompt.txt)<br/><i>"Форматируйте ответ, указывайте номер заказа, перечень позиций и контактные данные."</i>]
    C[Шаблон ответа<br/>(03_response_template.txt)<br/><i>"Номер заказа, список позиций, сроки выполнения"</i>]
    D[Генерация ответа<br/>(Ollama)]
    E[Запись ответа с уникальным номером<br/>(04_model_response.txt)]
    F[Маршрутизация<br/>Отправка ответа пользователю или в отдел заказов]
